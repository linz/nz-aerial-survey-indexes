DROP TABLE IF EXISTS grid_trial_5k;

CREATE TEMPORARY TABLE grid_trial_5k AS (
    SELECT
    ...
    FROM
        aerial_lds. "nz_5k_tile_index") j
    WHERE
        vertex != npoints
);

ALTER TABLE grid_trial_5k
    ALTER COLUMN shape TYPE geometry(point, 2193);

ALTER TABLE grid_trial_5k
    ADD CONSTRAINT grid_trial_5k_pk PRIMARY KEY (id);

ALTER TABLE grid_trial_5k
    ADD COLUMN intersection_count int NOT NULL DEFAULT 1;

CREATE INDEX sidx_grid_trial_5k_shape ON grid_trial_5k USING gist (shape);
...
` DROP TABLE IF EXISTS "5k_merged_tiles";

CREATE TEMPORARY TABLE "5k_merged_tiles" AS
SELECT
    ST_Simplify (ST_Normalize (ST_Union (shape)), 0.1) AS shape
FROM
    aerial_lds. "nz_5k_tile_index";
...
` should be 50900 as it is all the corners for each shape`
