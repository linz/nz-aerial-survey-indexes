DROP TABLE IF EXISTS grid_trial_1k;

CREATE TEMPORARY TABLE grid_trial_1k AS (
    SELECT
    ...
    FROM
        aerial_lds. "nz_1k_tile_index") j
    WHERE
        vertex != npoints
);

ALTER TABLE grid_trial_1k
    ALTER COLUMN shape TYPE geometry(point, 2193);

ALTER TABLE grid_trial_1k
    ADD CONSTRAINT grid_trial_1k_pk PRIMARY KEY (id);

ALTER TABLE grid_trial_1k
    ADD COLUMN intersection_count int NOT NULL DEFAULT 1;

CREATE INDEX sidx_grid_trial_1k_shape ON grid_trial_1k USING gist (shape);
...
FROM (
    ...
    GROUP BY
        pn.id) AS subquery
WHERE
    grid_trial_1k.id = subquery.id;

`make single geom so then can ask if touches or within` CREATE TEMPORARY TABLE IF NOT EXISTS merged_tiles AS
SELECT
...
`
merge the polys and check they have same shape as the original merged 50k
` DROP TABLE IF EXISTS "1k_merged_tiles";

CREATE TEMPORARY TABLE "1k_merged_tiles" AS
SELECT
    ST_Simplify (ST_Normalize (ST_Union (shape)), 0.1) AS shape
FROM
    aerial_lds. "nz_1k_tile_index";
...
` should be 1272500. sheets 509 * vert dif per sheet 50 * horizont dif per sheet 50`
SELECT
    count(*) / 4
FROM
    grid_trial_1k;

` should be 1272500 as it is all the corners for each shape`
