DROP TABLE IF EXISTS grid_trial_10k;

CREATE TEMPORARY TABLE grid_trial_10k AS (
    SELECT
    ...
    FROM
        aerial_lds. "nz_10k_tile_index") j
    WHERE
        vertex != npoints
);

ALTER TABLE grid_trial_10k
    ALTER COLUMN shape TYPE geometry(point, 2193);

ALTER TABLE grid_trial_10k
    ADD CONSTRAINT grid_trial_10k_pk PRIMARY KEY (id);

ALTER TABLE grid_trial_10k
    ADD COLUMN intersection_count int NOT NULL DEFAULT 1;

CREATE INDEX sidx_grid_trial_10k_shape ON grid_trial_10k USING gist (shape);
...
FROM (
    SELECT
        ...
        count(*) AS count
    FROM
        grid_trial_10k AS pn,
        grid_trial_10k AS pn2
    WHERE
        pn.id != pn2.id
...
` DROP TABLE IF EXISTS "10k_merged_tiles";

CREATE TEMPORARY TABLE "10k_merged_tiles" AS
SELECT
    ST_Simplify (ST_Normalize (ST_Union (shape)), 0.1) AS shape
FROM
    aerial_lds. "nz_10k_tile_index";
...
` should be 12725 as it is all the corners for each shape`
