...
COMMENT ON TABLE aerial_lds.nz_imagery_survey_index IS
...
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.imagery_survey_id IS
'Unique identifier for the aerial_lds.nz_imagery_survey_index table.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.name IS
'The corresponding aerial imagery layer name in the LINZ Data Service.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.imagery_id IS
'Foreign key to the aerial imagery layer identifier in the LINZ Data '
'Service.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.index_id IS
'Foreign key to the index tile layer identifier in the LINZ Data Service.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.set_order IS
...
'set.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.ground_sample_distance IS
'Ground sample distance in metres.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.accuracy IS
...
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.supplier IS
'The organisation that supplied the aerial imagery.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.licensor IS
'The organisation that licenses the aerial imagery.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.flown_from IS
'The earliest date on which aerial photographs were taken as part of this '
'imagery survey.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.flown_to IS
'The latest date on which aerial photographs were taken as part of this '
'imagery survey.';
COMMENT ON COLUMN aerial_lds.nz_imagery_survey_index.shape IS
'A dissolved, multipart boundary of the coverage of the imagery survey.';

CREATE TABLE IF NOT EXISTS aerial_lds.nz_elevation_survey_index (
...
COMMENT ON TABLE aerial_lds.nz_elevation_survey_index IS
'This table holds the standardised attributes of aerial elevation surveys.';

COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.elevation_survey_id IS
'Unique identifier for the aerial_lds.nz_elevation_survey_index table.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.name IS
'The corresponding aerial elevation layer name in the LINZ Data Service.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.index_id IS
'Foreign key to the index tile layer identifier in the LINZ Data Service.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.dem_id IS
'Foreign key to the DEM layer identifier in the LINZ Data Service.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.dsm_id IS
'Foreign key to the DSM layer identifier in the LINZ Data Service.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.point_cloud_doi IS
'Foreign key to the point cloud data identifier in Digital Object Identifyer https://doi.org';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.point_density IS
'The point density of the elevation survey.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.vertical_datum IS
'The vertical datum of the elevation survey.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.horizontal_datum IS
'The horizontal datum of the elevation survey.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.supplier IS
'The organisation that supplied the aerial elevation.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.licensor IS
'The organisation that licenses the aerial elevation.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.flown_from IS
'The earliest date on which aerial elevations were taken as part of this '
'elevation survey.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.flown_to IS
'The latest date on which aerial elevations were taken as part of this '
'elevation survey.';
COMMENT ON COLUMN aerial_lds.nz_elevation_survey_index.shape IS
